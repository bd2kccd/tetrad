...
public final class ColtDataSet implements DataSet, TetradSerializable {
    ...
    /**
     * The container storing the tetradMatrix. Rows are cases; columns are variables.
...
//                throw new IllegalArgumentException("Expecting all continuous variables: " + variables);
        ...
        ColtDataSet dataSet = new ColtDataSet(data.length, nodes);
        dataSet.tetradMatrix = new TetradMatrix(data);
...
*/
    ...
    public static DataSet serializableInstance() {
        ...
        ColtDataSet data = new ColtDataSet(1, variables);
...
//
//            for (int i = 0; i < tetradMatrix.rows(); i++) {
//                sum += tetradMatrix.get(i, j);
//            }
//
//            double avg = sum / tetradMatrix.rows();
//
//            for (int i = 0; i < tetradMatrix.rows(); i++) {
//                tetradMatrix.set(i, j, tetradMatrix.get(i, j) - avg);
//            }
//        }
//
//        TetradMatrix cov = TetradAlgebra.times(tetradMatrix.transpose(), tetradMatrix);
//
//        cov.assign(Mult.div(tetradMatrix.rows() - 1));
//
//        TetradMatrix cov = TetradMatrix.instance(tetradMatrix.columns(), tetradMatrix.columns());
//
//        for (int i = 0; i < tetradMatrix.columns(); i++) {
//            for (int j = 0; j < tetradMatrix.columns(); j++) {
//                cov.set(i, j, StatUtils.covariance(tetradMatrix.viewColumn(i).toArray(), tetradMatrix.viewColumn(j).toArray()));
//            }
//        }
//
//        return cov;
...
//                        sum += m * (entry1 - avg[j1]) * (entry2 - avg[j2]);
//                        n2 += m;
//                    }
//                }
//
//                cov.set(j1, j2, sum / (n2 - 1));
//            }
//        }
//
//        return cov;
...
//            for (int j2 = j1; j2 < tetradMatrix.columns(); j2++) {
//                double sum = 0.0;
//
//                for (int i = 0; i < tetradMatrix.rows(); i++) {
//                    double entry1 = tetradMatrix.get(i, j1);
...
//                cov.set(j2, j1, sum / (n - 1));
//            }
//        }

        TetradMatrix matrix = new TetradMatrix(tetradMatrix);
        ...
        return matrix.transpose().times(matrix).scalarMult(1.0 / matrix.rows());

//        double[][] doubles = tetradMatrix.toArray();
//        DenseDoubleMatrix2D denseDoubleMatrix2D = new DenseDoubleMatrix2D(doubles);
//        return new TetradMatrix(Statistic.covariance(denseDoubleMatrix2D).toArray());
    }
