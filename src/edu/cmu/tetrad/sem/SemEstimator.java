...
//        }
        ...
        TetradLogger.getInstance().log("stats", "Final FML = " + nf.format(semIm.getFml()));
...
//        optimizer = new SemOptimizerNrPowell();
        ...
        if (containsLatent && !(getSemPm().getGraph().existsDirectedCycle() ||
                containsCovarParam(getSemPm()))) {
            optimizer = new SemOptimizerEm();
//            optimizer = new SemOptimizerPalCds();
        } else if (containsFixedParam() || getSemPm().getGraph().existsDirectedCycle() ||
                containsCovarParam(getSemPm())) {
//            optimizer = new SemOptimizerScattershot();
