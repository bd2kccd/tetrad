...
//        }
    ...
    /**
     * Returns an array containing the getModel values for the free parameters,
...
//            if (node.getNodeType() == NodeType.ERROR) {
            ...
            int index = variableNodes.indexOf(node);
            TetradMatrix impliedCovar = getImplCovar();
...
//            e.printStackTrace();
        ...
        TetradMatrix identity = TetradAlgebra.identity(s.rows()); //TetradMatrix.identity(s.rows());
...
//        double penalty = getSemIm().getDof() * Math.log(sampleSize);           i
    ...
    public double getPValue() {
        ...
        if (dof <= 0) return Double.NaN;
            ...
            double chisq = new ChiSquaredDistribution(dof).cumulativeProbability(chiSquare);// ProbUtils.chisqCdf(chiSquare, dof);
            return 1.0 - chisq;
        }
    }
...
//                            Math.sqrt(errCovar().get(col, col)));
    ...
    public DataSet simulateDataReducedForm(int sampleSize, boolean latentDataSaved) {
        ...
        // into account!
//        TetradMatrix errCovar = TetradMatrix.identity(edgeCoef.rows());
//        TetradMatrix cholesky = MatrixUtils.choleskyC(errCovar);
        TetradMatrix cholesky = MatrixUtils.choleskyC(errCovar());
...
//            TetradVector e = new TetradVector(errCovar.rows());
//            for (int  i = 0; i < e.size(); i++) {
//                e.set(i, RandomUtil.getInstance().nextNormal(0, 1));
//            }

            TetradVector sample = inv.times(e);
...
//            }
//
//            if (!allEigenvaluesAreSmallerThanOneInModulus(edgeCoefC().transpose())) {
//                continue;
//            }
...
//    }
    ...
    public double getFgls() {
        ...
        double v = 0.5 * (diff.times(diff)).trace();
