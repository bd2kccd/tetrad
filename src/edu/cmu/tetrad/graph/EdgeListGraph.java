...
public class EdgeListGraph implements Graph {
    ...
    /**
     * A path matrix. The nodes are in dpathNodes.
     */
    private transient int[][] dpath;
    private transient List<Node> dpathNodes;
    private transient Map<Node, Integer> dpathNodesIndices;
...
//        GraphUtils.circleLayout(this, 200, 200, 150);
    ...
    public static Graph shallowCopy(EdgeListGraph graph) {
        ...
        _graph.edges = new ArrayList(graph.edges);
...
//                System.out.println("u = " + u);
    ...
    private Set<Node> zAncestors3(List<Node> z) {
        reconstituteDpath();
        Set<Node> ancestors = new HashSet<Node>();

        for (Node _x : dpathNodes) {
            int x = dpathNodesIndices.get(_x);

            for (Node _y : z) {
                int y = dpathNodesIndices.get(_y);
    ...
    private void reconstituteDpath() {
        ...
        dpathNodesIndices = new HashMap<Node, Integer>();
        for (int i = 0; i < dpathNodes.size(); i++) dpathNodesIndices.put(dpathNodes.get(i), i);

        for (int i = 0; i < dpathNodes.size(); i++) {
            dpath[i][i] = 1;
        }

        for (int u = 0; u < getEdges().size(); u++) {

            for (Edge edge : getEdges()) {
                Node _node1 = Edges.getDirectedEdgeTail(edge);
                Node _node2 = Edges.getDirectedEdgeHead(edge);

                int _index1 = dpathNodesIndices.get(_node1);
                int _index2 = dpathNodesIndices.get(_node2);
                dpath[_index1][_index2] = 1;

                for (int i = 0; i < dpathNodes.size(); i++) {
                    if (dpath[i][_index1] == 1 && dpath[i][_index2] == 0) {
                        dpath[i][_index2] = 1;
                    }
                }
            }
     ...
     * up twice in the list of adjacents for X, for optimality; simply create a list an and array from these to
...
//                    node.getName() + " has already been added to the graph.");
    ...
    public boolean equals(Object o) {
        ...
        if (o instanceof EdgeListGraph) {
            ...
            return (_o.nodes.equals(this.nodes) && _o.edges.equals(this.edges));
        } else {
            Graph graph = (Graph) o;
