...
public final class GraphUtils {
    ...
    public static void distanceFromSelected(LayoutEditable layoutEditable) {
        DistanceFromSelected layout = new DistanceFromSelected(layoutEditable);
        layout.doLayout();
    }

    public static void causalOrder(LayoutEditable layoutEditable) {
        CausalOrder layout = new CausalOrder(layoutEditable);
        layout.doLayout();
    }

    public static void brainSpecial(LayoutEditable layoutEditable) {
        BrainSpecial layout = new BrainSpecial(layoutEditable);
        layout.doLayout();
    }
    ...
    public static Dag randomDag(List<Node> nodes, int numLatentConfounders,
        ...
        for (int i = 0; i < numLatentConfounders; i++) {
            if (i >= commonCauses.size()) break;
            Node node = commonCauses.get(i);
            node.setNodeType(NodeType.LATENT);
            numNodesMadeLatent++;
        }

//        while (numNodesMadeLatent < numLatentConfounders) {
...
//        }
    ...
    private static List<Node> getCommonCauses(Dag dag) {
     ...
     */
    public static Dag randomDag(int numNodes, int numLatentConfounders,
                                int maxNumEdges, int maxDegree,
                                int maxIndegree, int maxOutdegree,
                                boolean connected) {
        if (numNodes <= 0) {
    ...
    public static Dag randomWattsStrogatz(int N, int K2, double beta) {
        ...
        for (int i = 0; i < N; i++) {
            for (int k = 1; k <= K2; k++) {
                int j = (i - k) % N;
                if (j < 0) j += N;
                addEdge(nodes, graph, i, j);
            }
        }

        for (Edge edge : graph.getEdges()) {
            if (RandomUtil.getInstance().nextDouble() > beta) continue;
            ...
            while (true) {
                k = RandomUtil.getInstance().nextInt(N);
                if (k == i) continue;
//                if (graph.isAdjacentTo(nodes.get(i), nodes.get(k))) continue;
                break;
            }

            graph.removeEdge(nodes.get(i), nodes.get(j));
            graph.removeEdge(nodes.get(i), nodes.get(k));
            addEdge(nodes, graph, i, k);
        }

        return graph;
    }

    private static void addEdge(List<Node> nodes, Graph graph, int i, int j) {
        if (graph.isAdjacentTo(nodes.get(i), nodes.get(j))) {
            throw new IllegalArgumentException("Nodes already adjacent: " + Edges.undirectedEdge(nodes.get(i), nodes.get(j)));
        }
     ...
     */
    public static Dag randomDagB(int numNodes, int numLatentConfounders,
     ...
     */
    public static Dag randomDagC(int numNodes, int numLatentConfounders,
                                 int numEdges) {
        if (numNodes <= 0) {
            throw new IllegalArgumentException(
                    "NumNodes most be > 0: " + numNodes);
...
//            }
    ...
    public static Graph randomDagCb(int numNodes, int numLatentConfounders,
                                    int numEdges) {
        if (numNodes <= 0) {
            throw new IllegalArgumentException(
                    "NumNodes most be > 0: " + numNodes);
...
//            }
     ...
     * nodes toward the end of the list.
     */
    public static Dag randomDagE(int numNodes, int numLatentConfounders,
...
//            System.out.println(node1 + "-->" + node2);
     ...
     * Directed Graphs," with optional biases added.
     */
    public static Dag randomDagD(int numNodes, int numLatentConfounders,
...
//        }
    ...
    public static Graph randomSingleFactorModel(int numStructuralNodes,
        ...
        do {
            dag = GraphUtils.randomDag(numStructuralNodes, numStructuralNodes, numStructuralEdges, 4, 3, 3, false);
...
//            System.out.println("Bidirected: " + graph.getEdge(measure1, measure2));
    ...
    public static Graph randomBifactorModel(int numStructuralNodes,
                                            ...
                                            int numMeasuredMeasuredImpureParents,
                                            int numMeasuredMeasuredImpureAssociations) {
        Graph mim = GraphUtils.randomSingleFactorModel(numStructuralNodes, numStructuralEdges,
...
//        System.out.println("Num arrowpoints = " + numArrowpts);
    ...
    public static GraphComparison getGraphComparison(Graph graph, Graph trueGraph) {
        ...
        TwoCycleErrors twoCycleErrors = getTwoCycleErrors(trueGraph, graph);
...
//        System.out.println("true graph = " + trueGraph);
        ...
        int twoCycleCorrect = twoCycleErrors.twoCycCor;
        int twoCycleFn = twoCycleErrors.twoCycFn;
        int twoCycleFp = twoCycleErrors.twoCycFn;
...
//                System.out.println(line);
    ...
    public static boolean existsDirectedPathFromTo(Graph graph, Node node1, Node node2) {
        return existsDirectedPathVisit(graph, node1, node2, new LinkedList<Node>(), 1000);
    }

    public static boolean existsDirectedPathFromTo(Graph graph, Node node1, Node node2, int depth) {
        return existsDirectedPathVisit(graph, node1, node2, new LinkedList<Node>(), depth);
    }

    public static boolean existsSemidirectedPathFromTo(Graph graph, Node node1, Node node2) {
        return existsSemiDirectedPathVisit(graph, node1, node2, new LinkedList<Node>(), 1000);
    }

    public static boolean existsSemidirectedPathFromTo(Graph graph, Node node1, Node node2, int maxDepth) {
        return existsSemiDirectedPathVisit(graph, node1, node2, new LinkedList<Node>(), maxDepth);
    }

    private static boolean existsDirectedPathVisit(Graph graph, Node node1, Node node2,
                                                   LinkedList<Node> path, int depth) {
        path.addLast(node1);
        ...
        for (Edge edge : graph.getEdges(node1)) {
            ...
            if (existsDirectedPathVisit(graph, child, node2, path, depth)) {
                return true;
            }
        }
    ...
    public static boolean existsSemiDirectedPathVisit(Graph graph, Node node1, Node node2,
                                                      LinkedList<Node> path, int maxDepth) {
        path.addLast(node1);
        Node previous = null;
...
//            }
            ...
            if (existsSemiDirectedPathVisit(graph, child, node2, path, maxDepth)) {
                return true;
            }
        }
...
//            }
    ...
    public static TwoCycleErrors getTwoCycleErrors(Graph trueGraph, Graph estGraph) {
        List<Edge> trueEdges = trueGraph.getEdges();
        ...
        List<Edge> estEdges = estGraph.getEdges();
