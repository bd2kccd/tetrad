...
//                    System.out.println("DEPENDENCY " + SearchLogUtils.independenceFact(x, y, empty) + " p = " +
//                            nf.format(test.getPValue()));
//                    System.out.println(" # edges = " + countAdjacencies(adjacencies));
                }
            }
...
//    }
    ...
    private boolean searchAtDepth(List<Node> nodes, IndependenceTest test, Map<Node, Set<Node>> adjacencies, int depth) {
        ...
        for (Node x : nodes) {
            ...
            EDGE:
            for (Node y : adjx) {
                List<Node> _adjx = new ArrayList<Node>(adjacenciesCopy.get(x));
                _adjx.remove(y);
