 ...
 */
public final class IndTestMultiFisherZ4 implements IndependenceTest {
    ...
    public IndTestMultiFisherZ4(Map<Node, List<Node>> nodeMap, Map<Node, TetradMatrix> coords, CovarianceMatrix cov,
...
//        CA1          50         -70 15 90     2
    ...
    private int countAboveThreshold(int[][][] X, int threshold) {
        ...
        for (int x = 0; x < X.length; x++) {
            for (int y = 0; y < X[0].length; y++) {
                for (int z = 0; z < X[0][0].length; z++) {
                    if (X[x][y][z] >= 0 && X[x][y][z] >= threshold) {
                        count++;
                    }
                }
            }
