 ...
 */
public class FasStableConcurrent {
    ...
    /**
     * The independence test. This should be appropriate to the types                               g
     */
...
//    private List<Double> pValues = new ArrayList<Double>();
     ...
     */
    public FasStableConcurrent(Graph graph, IndependenceTest test) {
        this.graph = graph;
        this.test = test;
    }
    ...
    public Graph search() {
        ...
        for (Node node : nodes) {
            adjacencies.put(node, new ConcurrentSkipListSet<Node>());
        }
...
//                System.out.println(" # edges = " + countAdjacencies(adjacencies));
//            }
...
//                        System.out.println("Trying " + x + " _||_ " + y);
                        ...
                        if (independent && noEdgeRequired) {
                            getSepsets().set(x, y, Collections.EMPTY_LIST);
//                            getLogger().log("independencies", SearchLogUtils.independenceFact(x, y, empty) + " p = " +
...
//        return freeDegree(nodes, adjacencies) > 0;
    ...
    private int countAdjacencies(Map<Node, Set<Node>> adjacencies) {
        int count = 0;

        for (Node node : adjacencies.keySet()) {
            for (Node _node : adjacencies.get(node)) {
                count++;
            }
        }
...
//    }
    ...
    private boolean searchAtDepth(final List<Node> nodes, final IndependenceTest test, final Map<Node, Set<Node>> adjacencies,
        ...
        Map<Node, Set<Node>> adjacenciesCopy = new ConcurrentHashMap<Node, Set<Node>>();

        for (Node node : adjacencies.keySet()) {
            adjacenciesCopy.put(node, new ConcurrentSkipListSet<Node>(adjacencies.get(node)));
        }
        ...
        for (Node _x : nodes) {
            ...
            if (++count % 100 == 0) {
                if (verbose) {
                    System.out.println("count " + count + " of " + nodes.size());
                    System.out.println(" # edges = " + countAdjacencies(adjacencies));
                }
            }
            ...
            for (Node _y : adjx) {
                ...
                final List<Node> _adjx = new ArrayList<Node>(adjacenciesCopy.get(x));
                _adjx.remove(y);
                final List<Node> ppx = possibleParents(x, _adjx, knowledge);
...
//                                    getLogger().log("independencies", SearchLogUtils.independenceFact(x, y, condSet) + " p = " +
//                                            nf.format(test.getPValue()));
...
//                            System.out.println("DEPENDENCY " + SearchLogUtils.independenceFact(x, y, condSet) + " p = " +
//                                    nf.format(test.getPValue()));
