...
public final class Cci {
    ...
    public boolean isIndependent(String x, String y, List<String> z) {
        double[][] rXYZ = residuals(x, y, z);
        return independent(rXYZ[0], rXYZ[1]);
...
//                double t2 = 1; // If x, y independent --> corr = 0
        ...
        Collections.sort(p);
        double cutoff = fdrCutoff(alpha, p, true);
...
//        return minP > cutoff;
    ...
    public double[][] residuals(String x, String y, List<String> z) {
        ...
        if (z.size() == 0) {
            for (int i = 0; i < N; i++) {
                residualsx[i] = data.getEntry(i, _x);
            }

            for (int i = 0; i < N; i++) {
                residualsy[i] = data.getEntry(i, _y);
            }
        ...
        for (int i = 0; i < N; i++) {
            for (int j = i + 1; j < N; j++) {
                ...
                sumsx[i] += k * data.getEntry(j, _x);
                sumsx[j] += k * data.getEntry(i, _x);
                sumsy[i] += k * data.getEntry(j, _y);
                sumsy[j] += k * data.getEntry(i, _y);
        ...
        for (int i = 0; i < N; i++) {
            ...
            sumsx[i] += k * data.getEntry(i, _x);
            sumsy[i] += k * data.getEntry(i, _y);

            weights[i] += k;
        }

        for (int i = 0; i < N; i++) {
            residualsx[i] = data.getEntry(i, _x) - sumsx[i] / weights[i];
            residualsy[i] = data.getEntry(i, _y) - sumsy[i] / weights[i];
        }
