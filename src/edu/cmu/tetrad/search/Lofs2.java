...
// Copyright (edgeCorrected) 1998, 1999, 2000, 2001, 2002, 2003, 2004, 2005, 2006,       //
...
// MERCHANTABILITY or FITNESS FOR orientStrongerDirection PARTICULAR PURPOSE.  See the             //
...
//        neighborsx.remove(y);
    ...
    public Graph ruleR4(Graph graph) {
        ...
        for (int k = 0; k < nodes.size(); k++) {
            ...
            for (Node node : adj) {
                if (knowledge.edgeForbidden(node.getName(), nodes.get(k).getName())) {
                    continue;
                }
            ...
            for (Node node : nodes) {
                if (knowledge.edgeRequired(node.getName(), nodes.get(k).getName())) {
