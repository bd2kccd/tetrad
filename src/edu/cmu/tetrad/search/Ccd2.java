...
public final class Ccd2 implements GraphSearch {
     ...
     */
    public Ccd2(IndependenceTest test, Knowledge knowledge) {
     ...
     * background knowledge will be done later.
     */
    public Ccd2(IndependenceTest test) {
    ...
    public Graph search() {
        ...
        Graph phi = new EdgeListGraph(nodes);
...
//        IFas search = new FasICov(phi, (IndTestFisherZ) test);
        Fas search = new Fas(phi, test);
        search.setDepth(_depth);
        search.setKnowledge(getKnowledge());
...
//                }
                ...
                List<Node> sepset = sepsets.get(anode, cnode);

                // Adding in the guy in the middle, poor man's CPC style.
                List<Node> sepset2 = new ArrayList<Node>(sepset);

                if (!sepset2.contains(anode)) {
                    sepset2.add(anode);
                    double alpha = test.getAlpha();
                    test.setAlpha(alpha);

                    if (test.isIndependent(bnode, cnode, sepset2)) {
                        sepset = sepset2;
                        sepsets.set(bnode, cnode, sepset);
                    }
                }

                if (!sepset.contains(bnode)) {
