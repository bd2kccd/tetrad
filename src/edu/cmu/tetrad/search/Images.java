...
//        }
        ...
        if (getKnowledge().edgeForbidden(x.getName(), y.getName())) {
            return;
        }
...
//
//        for (int d = 0; d < numDataSets(); d++) {
//            scores.add(localSemScoreOneDataSet(d, i, parents));
//        }
//
...
//        int count = 0;
//
//        for (int m = numDataSets() / 2; m < numDataSets(); m++) {
//            sum += scores.get(m);
//            count++;
//        }
//
//        return sum / count;
    ...
    private double localSemScoreOneDataSet(int dataIndex, int i, int[] parents) {
        ...
        int n = sampleSize();
        int k = ((parents.length + 1) * (parents.length + 2)) / 2;
...
//        return -n * log(variance) - c * k * log(n);
        return -n * Math.log(residualVariance) - n * Math.log(2 * Math.PI) - n - k * c * (Math.log(n));
//        return -n * Math.log(residualVariance) - n * Math.log(2 * Math.PI) - n - k * c * (Math.log(n) - Math.log(2*Math.PI));
    }
    ...
    public String bootstrapPercentagesString(int numBootstraps) {
        ...
        builder.append(edgePercentagesString(counts, returnGraph.getEdges(), null, numBootstraps));
    ...
    public String averageStandardizedCoefficientsString(Graph graph) {
        ...
        return edgeCoefsString(coefs, graph.getEdges(), "Estimated adjacencyGraph",
    ...
    public String logEdgeBayesFactorsString(Graph dag) {
        ...
        return logBayesPosteriorFactorsString(coefs, scoreGraph(dag));
    }
    ...
    private String gesEdgesString(Map<Edge, Integer> counts, List<DataSet> dataSets) {
        ...
        return edgePercentagesString(counts, returnGraph.getEdges(),
