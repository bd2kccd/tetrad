...
//            residuals.viewColumn(i).assign(residualsColumn);
    ...
    public static DataSet ar3(DataSet timeSeries) {
        TetradMatrix _timeSeries = timeSeries.getDoubleData();

        TetradMatrix residuals = new TetradMatrix(_timeSeries.rows() - 1, timeSeries.getNumColumns());

        for (int j = 0; j < _timeSeries.columns(); j++) {
            TetradMatrix x = _timeSeries.getPart(1, j, _timeSeries.rows() - 1, 1);
            TetradMatrix y = _timeSeries.getPart(0, 0, _timeSeries.rows() - 1, _timeSeries.columns());
...
//            residuals.viewColumn(j).assign(res.viewColumn(0));
            residuals.assignColumn(j, res.getColumn(0));
        }
...
//            residuals.viewColumn(i).assign(residualsColumn);
    ...
    public static DataSet createLagData(DataSet data, int numLags) {
        ...
        Knowledge knowledge = new Knowledge(); // todo switch back to Knowledge
