        ...
        for (int i = 0; i < nodes.size(); i++) {
            ...
            for (int j = i + 1; j < nodes.size(); j++) {
                ...
                try {
                    ...
                    if (independent) {
                        if ((test.determines(Collections.singletonList(x), y) || test.determines(Collections.singletonList(y), x))) {
                            independent = false;
                        }
                    }
                } catch (Exception e) {
                ...
                if (independent) {
                    if (noEdgeRequired) {
                        ...
                        this.logger.log("independencies", SearchLogUtils.independenceFact(x, y, empty) + " p = " +
                                nf.format(test.getPValue()));
                        System.out.println(SearchLogUtils.independenceFact(x, y, empty) + " p = " +
                                nf.format(test.getPValue()));
                    }
                } else if (!forbiddenEdge(x, y)) {
                    this.logger.log("allowing edge ", SearchLogUtils.independenceFact(x, y, empty) + " p = " +
                            nf.format(test.getPValue()));
                    adjacencies.get(x).add(y);
                    adjacencies.get(y).add(x);

                    System.out.println(makeGraph(adjacencies));
                }
            }
        }
...
//                            if (independent) {
//                                Set<Node> set = new HashSet<Node>(condSet);
//                                set.add(x);
//                                set.add(y);
//
//                                for (Node node : set) {
//                                    set.remove(node);
//                                    if (test.determines(new ArrayList<Node>(set), node)) {
//                                        independent = false;
//                                        break;
//                                    }
//                                }
//                            }
                            ...
                            if (independent) {
                                if (test.determines(Collections.singletonList(x), y)
                                        || test.determines(Collections.singletonList(y), x)
                                        || test.determines(condSet, x)
                                        || test.determines(condSet, y)) {
                                    independent = false;
                                }
                            }
                        } catch (Exception e) {
                        ...
                        if (independent) {
                            if (noEdgeRequired) {
                                ...
                                this.logger.log("independencies", SearchLogUtils.independenceFact(x, y, condSet) + " p = " +
                                        nf.format(test.getPValue()));
                            ...
                            System.out.println("allowing edge " + SearchLogUtils.independenceFact(x, y, condSet) + " p = " +
                                    nf.format(test.getPValue()));
...
//        return numRemoved > 0;
    ...
    private Graph makeGraph(Map<Node, Set<Node>> adjacencies) {
