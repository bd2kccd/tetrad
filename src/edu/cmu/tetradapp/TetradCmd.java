...
public final class TetradCmd {
    ...
    private TestType testType = TestType.TETRAD_BOLLEN;
    private Graph initialGraph;
    private boolean rfciUsed = false;
    private boolean rrfciUsed = false;
        ...
        while (tokenizer.hasToken()) {
            ...
            if ("-data".equalsIgnoreCase(token)) {
                ...
                if (argument.equals("delta")) {
                    testType = TestType.TETRAD_BOLLEN;
                ...
                this.rfciUsed = true;
            } else if ("-rrfci".equalsIgnoreCase(token)) {
                this.rrfciUsed = true;
...
//        TetradLogger.getInstance().setForceLog(false);
    ...
    private void printRandomDag() {
        ...
        do {
            dag = GraphUtils.randomDag(_numNodes, 0, _numEdges,
                    30, 15, 15, false);
...
//            TetradLogger.getInstance().setForceLog(true);
        ...
        if (rfciUsed && rrfciUsed) throw new IllegalArgumentException("Cannot set both " +
                "-rfci and -rrfci.");

        Fci fci = new Fci(getIndependenceTest());
        fci.setDepth(getDepth());
        fci.setKnowledge(getKnowledge());
        fci.setRFCI_Used(rfciUsed);
        fci.setRRFCI_Used(rrfciUsed);
