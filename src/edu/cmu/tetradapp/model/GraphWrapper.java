...
//            makeRandomGraph();
//        }
        if (!graphWrapper.allowRandomGraph()) {
            try {
                this.graph = new EdgeListGraph(graphWrapper.getGraph());
            } catch (Exception e) {
                e.printStackTrace();
    ...
    public GraphWrapper(DagWrapper dagWrapper) {
        if (Preferences.userRoot().getInt("newGraphInitializationMode", GraphParams.MANUAL) == GraphParams.MANUAL) {
    ...
    public GraphWrapper(SemGraphWrapper graphWrapper) {
        if (Preferences.userRoot().getInt("newGraphInitializationMode", GraphParams.MANUAL) == GraphParams.MANUAL) {
            this.graph = new EdgeListGraph(graphWrapper.getGraph());
        } else if (Preferences.userRoot().getInt("newGraphInitializationMode", GraphParams.MANUAL) == GraphParams.RANDOM) {
            makeRandomGraph();
    ...
    public GraphWrapper(TimeLagGraphWrapper graphWrapper) {
        this.graph = new EdgeListGraph(graphWrapper.getGraph());
        log();
    }
    ...
    public GraphWrapper(DagInPatternWrapper wrapper) {
        this(wrapper.getGraph());
    }

    public GraphWrapper(PValueImproverWrapper wrapper) {
        this(wrapper.getGraph());
    }
    ...
    private void makeRandomGraph() {
        if (Preferences.userRoot().getBoolean("makingRandomDags", false)) {
            makeRandomDag();
        } else if (Preferences.userRoot().getBoolean("makingRandomMims", false)) {
            makeRandomMim();
        } else {
            Preferences.userRoot().putBoolean("makingRandomDags", true);
            makeRandomDag();
        }
    ...
    private void makeRandomDag() {
        ...
        while (graph == null) {
            ...
            if (uniformlySelected) {
                ...
                dag = GraphUtils.randomDag(numMeasuredNodes + numLatents,
...
//                                    0);
//
////            Graph graph1 = new EdgeListGraph(parentGraph);
////            Graph graph1 = new MarshalledObject(parentGraph);
//            Graph graph1;
//
//            try {
//                graph1 = (Graph) new MarshalledObject(parentGraph).get();
//            }
//            catch (Exception e) {
//                throw new RuntimeException(e);
//            }
//
//            for (Node node : graph1.getNodes()) node.setNodeType(NodeType.LATENT);
//
//            Graph graph = GraphUtils.randomMim(graph1, measurementModelDegree,
//                    numLatentMeasuredImpureParents,
//                    numMeasuredMeasuredImpureParents,
//                    numMeasuredMeasuredImpureAssociations, true);
//
//            this.graph = graph;
//        } else

        {
            ...
            Graph graph = GraphUtils.randomSingleFactorModel(numStructuralNodes,
